...
const SupportNodeIcon = ({ active }: { active: boolean }) => (
  <svg viewBox="0 0 100 100" className="w-[80px] h-[80px] mx-auto md:w-[96px] md:h-[96px] shrink-0 drop-shadow-[0_0_15px_rgba(34,211,238,0.4)]">
    {/* Outer radar ping */}
    {active && [0, 1, 2].map((i) => (
      <motion.circle key={i} cx="50" cy="50" r="28" fill="none" stroke="#22d3ee" strokeWidth="1.5"
        animate={{ r: [28, 48], opacity: [0.8, 0] }} transition={{ duration: 1.5, repeat: Infinity, delay: i * 0.5, ease: "easeOut" }} />
    ))}

    {/* Spinning Scanner Target */}
    <motion.circle cx="50" cy="50" r="42" fill="none" stroke={active ? "rgba(34,211,238,0.4)" : "#333"} strokeWidth="1.5" strokeDasharray="30 60 10 60"
       animate={{ rotate: active ? 360 : 0 }} transition={{ duration: 10, repeat: Infinity, ease: "linear" }} style={{ transformOrigin: "center" }} />
    <motion.circle cx="50" cy="50" r="36" fill="none" stroke={active ? "rgba(34,211,238,0.15)" : "#222"} strokeWidth="4" strokeDasharray="2 8"
       animate={{ rotate: active ? -360 : 0 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} style={{ transformOrigin: "center" }} />

    {/* Central Core background */}
    <circle cx="50" cy="50" r="24" fill={active ? "rgba(34,211,238,0.1)" : "#0c0c0c"} stroke={active ? "#22d3ee" : "#333"} strokeWidth="2" className="transition-colors duration-500" />
    ...
    <g transform="translate(50, 48) scale(1)">
      <motion.path 
        ...
        fill={active ? "rgba(34,211,238,0.15)" : "none"} stroke={active ? "#fff" : "#888"} strokeWidth="2" strokeLinejoin="round"
        animate={{ y: active ? [0, -3, 0] : 0 }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} />
        
      {/* Dynamic Data Lines inside the chat bubble */}
      <motion.line x1="-8" y1="-3" x2="8" y2="-3" stroke={active ? "#22d3ee" : "#555"} strokeWidth="2" strokeLinecap="round" strokeDasharray="16"
        animate={{ strokeDashoffset: active ? [16, 0] : 0, y: active ? [0, -3, 0] : 0 }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} />
      <motion.line x1="-8" y1="3" x2="2" y2="3" stroke={active ? "#22d3ee" : "#555"} strokeWidth="2" strokeLinecap="round" strokeDasharray="10"
        animate={{ strokeDashoffset: active ? [10, 0] : 0, y: active ? [0, -3, 0] : 0 }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut", delay: 0.2 }} />
    </g>

    {/* Connecting dots (representing online operators) */}
    {active && (
      <g>
        <motion.circle cx="38" cy="38" r="3" fill="#10b981" animate={{ opacity: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 0.1 }} />
        <motion.circle cx="62" cy="62" r="3" fill="#10b981" animate={{ opacity: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 1.1 }} />
        <motion.circle cx="64" cy="34" r="3" fill="#10b981" animate={{ opacity: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 0.7 }} />
      </g>
    )}
  </svg>
);
    ...
    <section id="faq" className="py-24 sm:py-32 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <div className={`absolute inset-0 bg-[linear-gradient(to_right,rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:1rem_1rem] pointer-events-none transition-opacity duration-700 [mask-image:radial-gradient(ellipse_60%_60%_at_50%_20%,#000_10%,transparent_100%)] ${hoveredCTA ? "opacity-100" : "opacity-40"}`} />
          
          {/* Spotlight directly behind the floating SVG */}
          <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] bg-[radial-gradient(circle_at_50%_0%,rgba(34,211,238,0.12)_0%,transparent_70%)] pointer-events-none transition-opacity duration-1000 ${hoveredCTA ? "opacity-100" : "opacity-30"}`} />
          
          {/* Subtle horizontal top edge glow (Glassmorphism highlight) */}
          <div className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-white/40 to-transparent transition-colors duration-700 ${hoveredCTA ? "via-brand-neon" : "via-white/20"}`} />
          
          {/* Inner radial gradient that aggressively pulses on hover */}
          <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[radial-gradient(circle_at_50%_50%,rgba(34,211,238,0.06)_0%,transparent_50%)] pointer-events-none transition-opacity duration-1000 ${hoveredCTA ? "opacity-100" : "opacity-0"}`} />
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h3 className={`text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 tracking-tight transition-colors duration-500 ${hoveredCTA ? "text-white" : "text-zinc-200"}`}>
              {t('stillQuestions')}
            </h3>
            
            <p className={`text-base sm:text-lg mb-10 max-w-lg mx-auto font-medium transition-colors duration-500 ${hoveredCTA ? "text-zinc-400" : "text-zinc-500"}`}>
            ...
            >
              ...
              {hoveredCTA && (
                 <motion.div className="absolute inset-0 w-full h-[200%] bg-gradient-to-r from-transparent via-white/40 to-transparent skew-x-12 -translate-y-1/2"
                    animate={{ x: ["-100%", "200%"] }} transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }} />
              )}
              <span className="relative z-10 text-sm sm:text-base uppercase tracking-wider">{t('contactSupport')}</span>
